...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
    ...
    // Get existing student data to preserve fields not in form
    $existingStudent = null;
    if (isset($_SESSION['students'])) {
        foreach ($_SESSION['students'] as $student) {
            if ($student['id'] === $studentId) {
                $existingStudent = $student;
                error_log("Found student to update: " . print_r($existingStudent, true));
                break;
            }
        }
    }
    ...
    // Handle image upload
    $imagePath = isset($existingStudent['image']) ? $existingStudent['image'] : ''; // Keep old image by default
